...
final class EmailVerificationTests: XCTestCase {
    ...
    func testVerifyingEmailHappyPath() async throws {
        ...
        try await app.test(.GET, verifyURL) { req in
            try req.query.encode(["token": "token123"])
        } afterResponse: { res in
            XCTAssertEqual(res.status, .ok)
            let user = try await app.repositories.users.find(id: user.id!)
            let unwrappedUser = try XCTUnwrap(user)
            XCTAssertEqual(unwrappedUser.isEmailVerified, true)
            let token = try await app.repositories.emailTokens.find(userID: unwrappedUser.requireID())
            XCTAssertNil(token)
        }
    }
    ...
    func testVerifyingEmailWithExpiredTokenFails() async throws {
        ...
        try app.test(.GET, verifyURL) { req in
            try req.query.encode(["token": "token123"])
        } afterResponse: { res in
    ...
    func testResendEmailVerification() async throws {
        ...
        try await app.test(.POST, verifyURL) { req in
